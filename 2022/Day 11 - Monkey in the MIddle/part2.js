...
import { create, all, min, xgcd } from 'mathjs'
import { parse } from 'path'
const config = {}
const math = create(all, config)

const numRounds = 10000
let worryMitigator = 1
  ...
  mLines.forEach((ml, mli, mla) => {
    ...
    switch (cmd) {
      case 'monkey':
        ...
        if (mi !== mIndex) {
          throw new Error(
            `Invalid Monkey number: Expected ${mIndex}, Actual ${mi}`
          )
        }
      ...
      case 'operation':
        m.inspectItem = makeInspectItemMethod(
          ml.match(/^operation: new = (.*)$/)[1]
        )
        break
      ...
      case 'test':
        ...
        const mFalse = mla[mli + 2].match(
          /^if false: throw to monkey (\d+)$/
        )[1]
        m.throwItem = makeThrowItemMethod(d, mTrue, mFalse)
        worryMitigator *= parseInt(d)
        break
    }
  })
...
const topMonkeys = monkeys.sort(
  (m1, m2) => m2.itemsInspected - m1.itemsInspected
)
...
console.log(`\nTop Monkeys:\n\n${JSON.stringify(monkeys, monkeySubset())}`)

console.log(
  topMonkeys.slice(0, 2).reduce((total, m) => total * m.itemsInspected, 1)
)
